...
class ThemeNotifier extends Notifier<AppThemeType> {
  ...
  AppThemeType build() {
    ...
    final themeStr = settingsBox.get('app_theme');
    if (themeStr != null) {
      return AppThemeType.values.firstWhere((e) => e.toString() == themeStr, orElse: () => AppThemeType.darkBlue);
    }
  ...
  void setTheme(AppThemeType type) {
    state = type;
    final settingsBox = Hive.box('settings');
    settingsBox.put('app_theme', type.toString());
  }
}
...
class TaskNotifier extends Notifier<List<TaskModel>> {
  ...
  List<TaskModel> _loadTasks() {
    ...
    // Cleanup script for accumulated water tasks from old bug
    final cleanupKeys = <dynamic>[];
    final today = DateTime.now();
    for (var task in tasks) {
      if (task.id.startsWith('water_') || (task.title == 'Drink Water' && task.isCustom)) {
        if (task.date.year != today.year || task.date.month != today.month || task.date.day != today.day) {
          cleanupKeys.add(task.key);
        }
      }
    }
    if (cleanupKeys.isNotEmpty) {
      box.deleteAll(cleanupKeys);
    ...
    DateTime? lastReset = lastResetStr != null ? DateTime.tryParse(lastResetStr) : null;
    DateTime now = DateTime.now();
    DateTime todayOneAM = DateTime(now.year, now.month, now.day, 1, 0, 0);
    
    DateTime currentBoundary = (now.isAfter(todayOneAM) || now.isAtSameMomentAs(todayOneAM))
        ? todayOneAM
        : todayOneAM.subtract(const Duration(days: 1));

    if (lastReset == null || lastReset.isBefore(currentBoundary)) {
      _saveDailyProgress(currentBoundary.subtract(const Duration(days: 1)), tasks);
  ...
  List<TaskModel> _seedDummyData(Box<TaskModel> box) {
    final dummyTasks = [
      TaskModel(id: '1', title: 'Morning Workout', date: DateTime.now(), isCustom: false),
      TaskModel(id: '2', title: 'Eat Breakfast', date: DateTime.now(), isCustom: false),
      TaskModel(id: '3', title: 'DSA Practice', date: DateTime.now(), isCustom: false),
      TaskModel(id: '4', title: 'AIML Learning', date: DateTime.now(), isCustom: false),
      TaskModel(id: '5', title: 'Full Stack Development', date: DateTime.now(), isCustom: false),
      TaskModel(id: '6', title: 'Drink Water', date: DateTime.now(), isCustom: false),
      TaskModel(id: '7', title: 'Evening Walk', date: DateTime.now(), isCustom: false),
      TaskModel(id: '8', title: 'Review Learning', date: DateTime.now(), isCustom: false),
      TaskModel(id: '9', title: 'Sleep on Time', date: DateTime.now(), isCustom: false),
    ];
  ...
  double get completionPercentage => state.isEmpty ? 0 : completedTasks / state.length;
}
...
class WorkoutNotifier extends Notifier<List<WorkoutModel>> {
  ...
  List<WorkoutModel> _loadWorkouts() {
    ...
    if (workouts.isEmpty || workouts.any((w) => ['Chest', 'Arms', 'Abs', 'Legs', 'Full Body'].contains(w.category))) {
  ...
  List<WorkoutModel> _seedDummyData(Box<WorkoutModel> box) {
    final dummyWorkouts = [
      // Monday
      WorkoutModel(id: 'w1', category: 'Monday', exerciseName: 'Push-ups 12 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w2', category: 'Monday', exerciseName: 'Incline Push-ups 10 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w3', category: 'Monday', exerciseName: 'Decline Push-ups 8 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w4', category: 'Monday', exerciseName: 'Crunches 20 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w5', category: 'Monday', exerciseName: 'Plank 30 sec', setsCompleted: 3, date: DateTime.now()),
      // Tuesday
      WorkoutModel(id: 'w6', category: 'Tuesday', exerciseName: 'Backpack Curls 15 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w7', category: 'Tuesday', exerciseName: 'Towel Curls 15 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w8', category: 'Tuesday', exerciseName: 'Diamond Push-ups 8 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w9', category: 'Tuesday', exerciseName: 'Leg Raises 15 reps', setsCompleted: 3, date: DateTime.now()),
      // Wednesday
      WorkoutModel(id: 'w10', category: 'Wednesday', exerciseName: 'Squats 20 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w11', category: 'Wednesday', exerciseName: 'Lunges 15 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w12', category: 'Wednesday', exerciseName: 'Jump Squats 12 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w13', category: 'Wednesday', exerciseName: 'Calf Raises 25 reps', setsCompleted: 3, date: DateTime.now()),
      // Thursday
      WorkoutModel(id: 'w14', category: 'Thursday', exerciseName: 'Push-ups 12 reps', setsCompleted: 4, date: DateTime.now()),
      WorkoutModel(id: 'w15', category: 'Thursday', exerciseName: 'Incline Push-ups 12 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w16', category: 'Thursday', exerciseName: 'Backpack Curls 15 reps', setsCompleted: 3, date: DateTime.now()),
      // Friday
      WorkoutModel(id: 'w17', category: 'Friday', exerciseName: 'Crunches 25 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w18', category: 'Friday', exerciseName: 'Bicycle Crunch 20 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w19', category: 'Friday', exerciseName: 'Plank 45 sec', setsCompleted: 3, date: DateTime.now()),
      // Saturday
      WorkoutModel(id: 'w20', category: 'Saturday', exerciseName: 'Push-ups 12 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w21', category: 'Saturday', exerciseName: 'Squats 20 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w22', category: 'Saturday', exerciseName: 'Lunges 15 reps', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w23', category: 'Saturday', exerciseName: 'Plank 30 sec', setsCompleted: 3, date: DateTime.now()),
      WorkoutModel(id: 'w24', category: 'Saturday', exerciseName: 'Mountain Climbers 20 reps', setsCompleted: 3, date: DateTime.now()),
    ];
...
final taskProgressProvider = Provider<Map<DateTime, double>>((ref) {
  ...
  for (var key in settingsBox.keys) {
    if (key is String && key.startsWith('task_progress_') && !key.startsWith('task_progress_details_')) {
...
final taskProgressDetailsProvider = Provider<Map<DateTime, Map<String, dynamic>>>((ref) {
  ...
  for (var key in settingsBox.keys) {
    if (key is String && key.startsWith('task_progress_details_')) {
      ...
      if (value is Map) {
        ...
        try {
          final date = DateTime.parse(dateStr);
          history[DateTime(date.year, date.month, date.day)] = Map<String, dynamic>.from(value.cast<String, dynamic>());
        } catch (_) {}
      }
    }
  }
...
final workoutCalendarInclusionProvider = NotifierProvider<WorkoutCalendarInclusionNotifier, Map<DateTime, bool>>(() {
